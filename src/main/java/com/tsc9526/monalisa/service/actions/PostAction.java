...
public class PostAction extends PutAction{
	...
	public Response insertTablesRows(){
		...
		for(String table:args.getTables()){
			...
			List<Record> records=ServletRequestParser.parseArrays(tpl, args.getReq(),getTableMapping(tb));
	...
	public Response insertTableRows(){
		...
		List<Record> records=ServletRequestParser.parseArrays(tpl, args.getReq());
		if(records.size()==1){
